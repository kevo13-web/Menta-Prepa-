...
export function AccountPlanningGenerator() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      const { data } = await supabase
        ...
        .eq("id", user.id)
        .maybeSingle();
      if (!cancelled && data) setProfile({ ...(data as Omit<AcademicProfile, "user_id">), user_id: user.id });
    };
    void loadProfile();
  ...
  const suggestedSubjects = useMemo(
    () => profile ? suggestedSubjectsForProfile(profile) : [],
    [profile],
  );
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                ...
                {suggestedSubjects.length ? (
                  <p className="mt-3 max-w-4xl text-xs leading-5 text-[#667b90]"><strong className="text-[#405c78]">Matières reconnues :</strong> {suggestedSubjects.join(" · ")}</p>
                ) : null}
