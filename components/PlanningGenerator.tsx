...
function buildSchedule(subjects: string[], fatigue: number, goal: string): DaySchedule[] {
  ...
  return [
    ...
    {
      day: "Mercredi",
      focus: "Milieu de semaine sans dispersion",
      blocks: [
    ...
    {
      day: "Vendredi",
      focus: "Fermer les boucles ouvertes",
      blocks: [
    ...
    {
      day: "Samedi",
      focus: goal || "Bloc long de progression",
      blocks: [
    ...
    {
      day: "Dimanche",
      focus: "Bilan et semaine suivante",
      blocks: [
    ...
    <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <GlassCard className="h-fit">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold text-frost">Données de semaine</h2>
        ...
        <div className="grid gap-4">
          <label className="grid gap-2 text-sm text-frost">
            ...
            <select
              ...
              className="rounded-lg border border-white/10 bg-ink/80 px-3 py-3 text-sm text-frost outline-none transition focus:border-mint/50"
            >
              {levels.map((item) => (
          ...
          <label className="grid gap-2 text-sm text-frost">
            ...
            <input
              ...
              className="rounded-lg border border-white/10 bg-ink/80 px-3 py-3 text-sm text-frost outline-none transition placeholder:text-muted focus:border-mint/50"
              placeholder="Philosophie, Maths, Histoire"
            />
          </label>
          ...
          <label className="grid gap-2 text-sm text-frost">
            ...
            <textarea
              ...
              rows={3}
              className="resize-none rounded-lg border border-white/10 bg-ink/80 px-3 py-3 text-sm text-frost outline-none transition placeholder:text-muted focus:border-mint/50"
            />
          </label>
          ...
          <label className="grid gap-2 text-sm text-frost">
            Devoirs, khôlles, examens
            <textarea
              value={deadlines}
              onChange={(event) => setDeadlines(event.target.value)}
              rows={3}
              className="resize-none rounded-lg border border-white/10 bg-ink/80 px-3 py-3 text-sm text-frost outline-none transition focus:border-mint/50"
            />
          </label>
          ...
          <PremiumButton onClick={() => setGenerated(true)} icon={Sparkles} className="mt-2 w-full">
      ...
      <div className="space-y-5">
        <motion.div
          initial={false}
          animate={generated ? { opacity: 1, y: 0 } : { opacity: 0.88, y: 0 }}
          className="rounded-lg border border-white/10 bg-white/[0.045] p-4"
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-mint">
                Planning {generated ? "personnalisé" : "exemple"}
              </p>
              <h2 className="mt-2 text-2xl font-semibold text-frost">{level}</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              {subjects.slice(0, 4).map((subject) => (
                <span key={subject} className="rounded-full border border-white/10 bg-white/8 px-3 py-1 text-xs text-muted">
                  {subject}
                </span>
              ))}
            </div>
          ...
          <div className="mt-4 grid gap-3 text-sm text-muted md:grid-cols-3">
            <div className="rounded-lg bg-ink/60 p-3">
              <GraduationCap className="mb-2 h-4 w-4 text-mint" />
              {courseHours}
            </div>
            <div className="rounded-lg bg-ink/60 p-3">{freeHours}</div>
            <div className="rounded-lg bg-ink/60 p-3">{deadlines}</div>
          </div>
        </motion.div>
