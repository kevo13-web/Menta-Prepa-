  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[#315b7f]/10 bg-[#f8f4ed]/82 backdrop-blur-2xl">
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <button type="button" aria-label="Ouvrir le menu" onClick={() => setOpen((value) => !value)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#315b7f]/15 bg-white/65 text-[#173a5e] shadow-sm lg:hidden">
      ...
      {open ? (
        <div className="border-t border-[#315b7f]/10 bg-[#f8f4ed]/96 px-4 py-4 backdrop-blur-2xl lg:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-2">
            ...
            <Link href="/focus" onClick={() => setOpen(false)} className={cn("inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#e6efff] to-[#e8f8f2] px-3 py-3 text-sm font-semibold text-[#315a8d]", pathname === "/focus" && "ring-2 ring-[#6e98ef]/25")}><Timer className="h-4 w-4" /> Focus Pomodoro</Link>
